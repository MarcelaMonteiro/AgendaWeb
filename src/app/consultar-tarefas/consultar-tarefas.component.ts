...
export class ConsultarTarefasComponent {
  ...
  onSubmit() {
    ...
    this.httpClient
      .get(environment.apiTarefas + 'api/tarefas/' + dataMin + '/' + dataMax)
      .subscribe({
        next: (data) => {
          //obtendo retorno de sucesso
          this.tarefas = data as any[]; //armazenando os dados obtidos da API
        },
        error: (e) => {
          //obtendo retorno de erro
