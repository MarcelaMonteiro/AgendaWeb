...
export class EditarTarefasComponent {
  ...
  ngOnInit() {
    ...
    this.httpClient
      .get(environment.apiTarefas + 'api/tarefas/' + this.id)
      .subscribe({
        next: (data: any) => {
          ...
          if (data.prioridade == 'Baixa')
            this.form.get('prioridade')?.setValue('1');
          else if (data.prioridade == 'Media')
            this.form.get('prioridade')?.setValue('2');
          else if (data.prioridade == 'Alta')
            this.form.get('prioridade')?.setValue('3');
        },
      });

    //fazendo uma requisição GET para o endpoint de consulta de categorias
    this.httpClient.get(environment.apiTarefas + 'api/categorias').subscribe({
      next: (data) => {
  ...
  onSubmit() {
    ...
    this.httpClient
      .put(environment.apiTarefas + 'api/tarefas/' + this.id, this.form.value)
      .subscribe({
        //aguardando a resposta da API
        next: (data: any) => {
          //capturando um objeto JSON da API
